...
[sources.json](sources.json); original copyright and BSD notice remain in pd.h.

Run `python tools/check_pd_line.py`. Its ten cases at each of 48/44.1 kHz compare
with an isolated [PD reference](line-control.pd). PD delivers control messages
...
emission counts must match exactly. See [results](line-results.json).

The existing `check_pd_primitives.py` audio-ramp fixture remains sample-identical
at both rates. [line-modes.cpp](line-modes.cpp) checks arity dispatch, held control
