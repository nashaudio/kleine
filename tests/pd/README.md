...
The shared control/audio [line API](line.md) adds 20 control fixtures and C++ mode/initializer checks. [Current topology review](../../farnell/KLANG-TOPOLOGY-REVIEW.md) supersedes old model-routing parity claims below; primitive fixtures remain independent.
...
sample-timed. See also the [54 model comparisons](vline-model-results.json).
...
| [lop.pd](lop.pd) | `pd::lop` | Unit impulse, 0.1/20/100 Hz |
| [bp.pd](bp.pd) | `pd::bpf` (existing class name for `bp~`) | Unit impulse: 2000 Hz/Q12, 400 Hz/Q7, 359 Hz/Q123, 632 Hz/Q400 |
...
| [vline.pd](vline.pd) | `pd::vline` | Complete float/list, cold-inlet, queue and stop contract; [separate runner and results](vline.md) |
...
| [vcf.pd](vcf.pd) | `pd::vcf` | Both quadrature outputs, 997 Hz input, 13 Hz modulation driving frequency through -3000 to 15000 Hz; Q=80 and zero-Q bypass |
| [samphold.pd](samphold.pd) | `pd::samphold` | Noise seed 404933, 137 Hz phasor trigger; capture on decreasing input |
| [rzero.pd](rzero.pd) | `pd::rzero` | Noise seed 404933, coefficient 0.99 |
...
simplified Detector. The [focused checks](env-output-results.json) compare raw
