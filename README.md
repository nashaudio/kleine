...
The **[Idiophonics trial](docs/farnell/audio/idiophonics.md)** completes chapters 29-33, including the earlier bell studies, Bouncing, Rolling, Creaking and Boing. It retains paired audio, website references, 33 comparisons per rate, source differences and reproduction commands.
...
See [AGENTS.md](AGENTS.md) for implementation principles, the evolving comparison process, and game priorities; see [farnell/README.md](docs/farnell/README.md) for reference provenance and PD-version findings.

Planning and review: [book/website coverage](docs/farnell/reviews/COVERAGE.md), [PD primitive usage and port status](docs/farnell/reviews/PD-PRIMITIVES.md), and [Klang issues and candidates](docs/farnell/reviews/KLANG-REVIEW.md). The coverage tables include planned files, figure/page references, dependencies, and the remaining mapping work.
...
The deliberately retained [chapter 25/29 listening bundle](docs/farnell/audio/README.md) contains website excerpts, PD bounces, Kleine bounces, and comparison evidence. See the [model documentation](docs/farnell/klang/README.md) for controls and reproduction commands.

The **[Artificial Sounds extended trial](docs/farnell/audio/artificial-sounds.md)** now covers chapters 24–28 and their bulk supplements. Its series guide links the Klang files, unprocessed website excerpts, paired bounces and 48/44.1 kHz validation. Reproduce it with `tools/render_artificial.py --retain`, `tools/render_artificial.py --rate 44100`, then `tools/package_artificial.py`. Two bulk phone demos use documented Vanilla reconstructions of missing list-library matchers; original patches are preserved.
...
The working `include/klang.h` now uses the repaired reflection OSM with
multi-cycle and fractional-duty support. The
[promotion report](docs/klang/OSM-IMPROVED.md) records validation,
performance trade-offs, model comparisons and the preserved previous header.
...
- [Isolated PD primitive tests](docs/tests/pd/README.md) check level, phase, seeded noise and FM parity at 48/44.1 kHz. The [website-level investigation](docs/farnell/audio/comparisons/reference-levels.md) separates recording gain from measured PD compatibility effects.
