...
- docs/farnell/README.md : reference sources, archive identity, and PD compatibility findings.
- docs/farnell/reviews/COVERAGE.md : full book/website patch work queue, figure/page mappings, dependencies, planned Klang paths and status; curated entries are in farnell/catalogue.json.
- docs/farnell/reviews/PD-PRIMITIVES.md : PD node usage and primitive port/validation status, keeping abstractions and external objects distinct.
- docs/farnell/reviews/KLANG-REVIEW.md : periodic review of Klang issues, gaps, prototypes and changes, with evidence, priority and effort estimates.
...
6. Klang has signal flow semantics but is immediate mode C++ - sample/buffer processing - not graph based.
   Klang must also be practical in Debug builds: make inexpensive ordinary sample paths and cached configuration explicit in the code, and measure Debug performance as well as Release. Do not rely solely on compiler inlining, constant folding or removal of redundant work.
...
11. Klang implementations of PD-Vanilla primitives can directly use the original C/C++ from the appropriate Pure Data repository, retaining its notices and recording the source revision. Aim for complete reusable primitive ports where possible, rather than helpers tailored to one model. Record missing behaviour explicitly and raise interface/topology difficulties for discussion. Review tilde/non-tilde pairs individually, favouring polymorphism in one class where appropriate; do not impose a namespace/suffix convention. For line, set(target, duration) selects audio behaviour and set(target, duration, grain) selects intervalled control output. Bit-perfect agreement is a useful diagnostic where source, arithmetic, state, and scheduling match; it is not a blanket requirement for a successful model.
12. Organise farnell/klang by section and practical name, e.g. Idiophonics/Telephone Bell/telephonebell.k. Use namespace pd and PD node names for primitive ports (pd::osc, pd::hip, pd::noise; existing pd::bpf represents bp~). Prefer matching instance names when unambiguous, e.g. pd::osc osc and osc(2500), so the model reads directly against the patch. Preserve hardcoded patch values unless configurable controls are requested as an adaptation.
13. In models prefer Klang param, signal, constant and appropriate unit helpers such as Frequency/dB. Every set(...) override must take only param or relative arguments, including switches and integer amounts, so Klang's overload dispatch works. C++ types are fine for internal state and PD source ports; prefer int unless unsigned or 64-bit values are necessary. Use Klang byte for unsigned char. Cache model parameters in set(); avoid configuring a child there when its inline process-time call already does so. Check for changed parameters before expensive reconfiguration (for example, recalculating filter coefficients); simple value-caching setters can skip that branch.
...
15. Inline single-use generators and short signal-flow chains; keep temporary signals when a sample is reused, evaluation order requires one, or a name explains a complex DSP stage. Use `out` directly for final-output accumulation when a separate signal adds no meaning and the previous output is not needed. Read an already evaluated object's `out` when appropriate instead of processing it again. Prefer clear Klang expressions such as parenthesised powers to manual multiplication loops, preserving the intended exponent. Put multiple statements in branches and functions on separate lines. Ask Chris about genuinely borderline conciseness/readability choices; his current preference is to inline short chains and retain names for complex stages.
16. Start from a pure model whose objects, constants, controls and signal flow map to its specific PD source patch. Use reusable PD primitives instead of model-local code simulating them. Keep recording observations, fitted performances and diagnostic controls in comparison tooling. Review any audible need for PD's 64-sample scheduling one sound at a time; an eventual workaround must be explicit in Klang and separable from the pure form.
17. Keep Doxygen-compatible API/usage comments and examples for PD primitives, covering parameter meanings and units, event versus per-sample use, source variants and remaining limits. Models should use minimal comments and self-documenting code: one short descriptive `//` comment before each object. A single longer Doxygen overview, with usage examples where useful, may appear near the top of a model file before its main Sound object; avoid repeated Doxygen blocks on components. Keep detailed comparison findings and port limitations in the model notes.

18. Keep alternate implementations of the same sound in separate development files or explicit compile-time options; do not bury website/bulk/historical switches in the model's runtime API or signal flow. Keep provenance and comparison labels in documentation and tooling. Eventually retain the preferred sounding implementation after listening. Selectors for distinct sounds or studies may remain for now.
19. Prefer the latest widely available companion-site patches and recordings. Where online, PD and Kleine agree perceptually, treat that as a sound-review pass; numerical differences alone do not require further work. Keep implementation/topology follow-ups distinct from sound acceptance. Phone Tones selections: 350/450 Hz dial, online smoothed busy (all busy fixtures accepted as perceptually equivalent), and the brighter ringback; the duller ALARMS handset variant is rejected.
...
Kleine supports deterministic chapter 25/29 bounces with `--render`, using its existing Processor without an audio device. `tools/render_farnell.py` runs paired PD/Kleine trials; `tools/package_farnell.py` retains the requested listening fixtures and comparison evidence in `farnell/audio/`. Scratch renders remain under `build/`. Engine::SIMULATED records MIDI event history and does not render audio offline. Build/run instructions are in README.md and docs/farnell/klang/README.md.

The extended Artificial Sounds trial (24–28) is documented in docs/farnell/audio/artificial-sounds.md. Use tools/render_artificial.py for its shared event recipes and tools/package_artificial.py for retained audio/evidence. The initial pd::line contract is block 64; pd::env implements the default 1024/512 analysis. New primitives have isolated fixtures. Combined phone references explicitly reconstruct missing list-library number matchers in render copies; never silently treat those copies as unchanged upstream sources. Techniques and the Pure Data teaching section are brought in when required by the current practical.

## Trial models and game priorities

Full book and companion-site coverage is the target, including introductory examples, practical subcomponents, and supplemental variants. Use stable chapter/practical plus patch IDs from docs/farnell/reviews/COVERAGE.md; figure numbers are references, not necessarily one-to-one patch identifiers. Maintain figure mapping confidence and validation evidence rather than inferring completion from a file's presence. Regenerate the coverage and primitive tables with tools/catalogue_farnell.py after updating farnell/catalogue.json or the source collection; exclude the parked farnell/pd/old archive from active coverage/usage counts.
...
The intended outcome is an open resource. The overall licence and remaining provenance are TBD, with MIT or a similar permissive licence under consideration; do not represent that intention as an already granted licence for all included material. Chris has confirmed Klang Open License 1.0 for the Klang language; its terms are in docs/licenses/Klang-Open-License-1.0.txt. Farnell's farnell/pd/old/AAA-README.FIRST expressly permits redistribution and modification of that collection's code and encourages attribution; do not extend its code permission to recordings or the book. Preserve upstream terms and attribution, document confirmed information in README.md, and keep the book local regardless of other material's licence.
