...
[Sixteen fixtures](../../../tests/pd/metro-results.json), eight at each of 48/44.1 kHz, compare cumulative counts against the isolated [PD patch](../../../tests/pd/metro.pd): default, fractional and fast periods; zero/negative periods; initially stopped state; interval change with a pending deadline; stop/restart/bang; numeric assignment including negative nonzero; and the legacy minimum-period rule. The first-poll `metro(100)` path is tested from a default-constructed object.
