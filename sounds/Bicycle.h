...
struct Bicycle : Sound {
	...
	void prepare() {
		param wheel_speed = controls[1].smooth();
		noise_lpf[0].set(2000 + wheel_speed * 100);
		noise_lpf[1].set(1000);
	}
	...
	void process() {
		pedalling += (min(1.f, controls[0].smooth()) - pedalling) * 0.00005;
		...
		signal chain_noise_filtered = chain_noise >> noise_lpf[1];
		...
		//signal pedal_weight = min(1.f, pedalling * 2);
		//signal chain_out = pedalling * chain_noise_filtered + (1 - pedalling) * wheel_noise * 0.5;
		signal click_out = 0.25 * wheel_noise >> click_bpf(min(fs.nyquist, 12500 + pedalling * wheel_speed * 100), 1);
		signal click_out_filtered = click_out >> noise_lpf[0];

		signal chain_out = softclip(chain_noise >> chain_bpf(3000, 1), 1, 0.25);

		signal click_out_clipped = softclip(click_out_filtered, 1.5, 0.5);

		pedalling * ((0.005 * wheel_speed * click_out_filtered) + 0.25 * click_out_clipped) + max(0.f, (1 - pedalling) * sqr(1 - wheel_speed * 0.001)) * click_out * 0.25 >> out;
		out *= 0.75;
	}
};