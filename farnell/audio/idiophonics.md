...
| --- | --- | --- | --- | --- |
| Bouncing | [bouncing.k](../klang/Idiophonics/Bouncing/bouncing.k) | [Online](30-bouncing/bouncing-online.wav) | [PD](30-bouncing/bouncing-pd.wav) / [Kleine](30-bouncing/bouncing-kleine.wav) | [Comparison](comparisons/idiophonics/bouncing.png) |
| Rolling can | [rolling.k](../klang/Idiophonics/Rolling/rolling.k) | [Online](31-rolling/rolling-online.wav) | [PD](31-rolling/rolling-pd.wav) / [Kleine](31-rolling/rolling-kleine.wav) | [Comparison](comparisons/idiophonics/rolling.png) |
| Struck can | `Rolling`, struck mode | Same practical | [PD](31-rolling/tincan-pd.wav) / [Kleine](31-rolling/tincan-kleine.wav) | [Comparison](comparisons/idiophonics/tincan.png) |
| Separate ground/can | `Rolling::Uneven` | Same practical | [PD](31-rolling/uneven2-pd.wav) / [Kleine](31-rolling/uneven2-kleine.wav) | [Comparison](comparisons/idiophonics/uneven2.png) |
| Creaking door | [creaking.k](../klang/Idiophonics/Creaking/creaking.k) | [Online](32-creaking/creaking-online.wav) | [PD](32-creaking/creaking-pd.wav) / [Kleine](32-creaking/creaking-kleine.wav) | [Comparison](comparisons/idiophonics/creaking.png) |
| Boing, website | [boing.k](../klang/Idiophonics/Boing/boing.k) | [Online](33-boing/boing-online.wav) | [PD](33-boing/boing-pd.wav) / [Kleine](33-boing/boing-kleine.wav) | [Comparison](comparisons/idiophonics/boing.png) |
...
| `ROLLING/uneven.pd` | `Rolling::Study` implements the audible random linear gestures and parabolic shaping: [PD](31-rolling/uneven-pd.wav) / [Kleine](31-rolling/uneven-kleine.wav). The noise/phasor experiments are disconnected from its audible output. The rendering copy explicitly enables its saved spigot control, starts the recursive delay loop, then stops it; the last value is held, so this diagnostic ground signal has DC and no silence tail. |
| `MRBOINGY/twang.pd` | With linear decay `d`, website `env16p` actually computes `3*d^64`. The bulk source computes `15^(d^4)` on modern PD; swapping the historical inlet convention gives `(d^4)^15`. [Bulk comparison](comparisons/idiophonics/boing-bulk.png), [legacy-convention comparison](comparisons/idiophonics/boing-legacy.png). Raw bulk output exceeds unity (about 1.015 at 48 kHz; 1.058 at 44.1 kHz) in both renderers. Quieter audition copies apply exactly 0.8 gain to both: [PD](33-boing/boing-bulk-pd-listening.wav) / [Kleine](33-boing/boing-bulk-kleine-listening.wav). Raw files remain available. |
| `BELL/A0`, `A1`, `A2` | [studies.k](../klang/Idiophonics/Telephone%20Bell/studies.k) uses PD oscillators, finite squared decay and `BellStudies::Partials`. A2's sum gain is 0.5, different from the final partial group: [PD](29-telephone-bell/bell-a2-pd.wav) / [Kleine](29-telephone-bell/bell-a2-kleine.wav). A0/A1 are diagnostic fixtures. |
| `BELL/A3-bell-ratio` / `A3-bell-scale` | `BellStudies::Ratios` preserves the double-burst cadence, unused even ringer outlet and disconnected middle overtone group: [PD](29-telephone-bell/bell-a3-pd.wav) / [Kleine](29-telephone-bell/bell-a3-kleine.wav). The dynamic numeric `$1`/`$2` object boxes **load successfully in PD 0.55.2**; the scale helper is exercised by the A3 render, with no reconstruction. The static dependency scanner still cannot expand arbitrary argument-named objects. |
| `BELL/A4-bell-telephone` | `BellStudies::Telephone` retains its different partial ratios, block-rounded linear/squared/fourth-power envelopes, noise strike and 59 ms body resonance: [PD](29-telephone-bell/bell-a4-pd.wav) / [Kleine](29-telephone-bell/bell-a4-kleine.wav). |
| `BELL/partial`, `group`, `testgroup` | Independent per-partial decays and the earlier 0.3333 group gain are covered separately from the final shared envelope. [Group PD](29-telephone-bell/bell-group-pd.wav) / [Kleine](29-telephone-bell/bell-group-kleine.wav), [testgroup PD](29-telephone-bell/bell-testgroup-pd.wav) / [Kleine](29-telephone-bell/bell-testgroup-kleine.wav). |
...
- The earlier Artificial Sounds fixtures pass again at both rates after the shared filter changes; the chapter 25/29 regression renders also remain close. [Smoke checks](comparisons/idiophonics/smoke-results.txt) exercise rapid control retargets and existing Harrier use of `pd::vcf`. Harrier required materialising its turbine output to compile; its selected smoke settings exceed unity substantially, so this establishes finite processing, not level safety or equivalence to its old timbre.

The shared [finite gesture/delay helpers](../klang/Idiophonics/common.h) cover these models. They are not complete `vline~`, named PD delay-buffer or general scheduler ports. Core `include/klang.h` remains unchanged; promotion and lifecycle work is recorded in [KLANG-REVIEW.md](../KLANG-REVIEW.md).
